...
/* ===================== Config ===================== */
#define TIME_SLICE_MS               50      // time slice หลักของระบบ time-sharing
#define RUN_EXPERIMENT_AT_START     1       // 1 = รันการทดลองก่อน, 0 = ข้ามไประบบปกติทันที

/* ===================== Globals (Stats) ===================== */
static uint32_t task_counter = 0;
static uint64_t context_switch_time = 0;    // เวลาที่ใช้ใน manual_scheduler() (งาน + overhead จำลอง)
...
static inline void manual_scheduler(void)
{
    ...
    // --- simulate context save/switch overhead (ก่อนรันงาน) ---
    context_switches++;
    ...
    // --- execute current task (round-robin) ---
    ...
    // --- simulate context restore overhead (หลังรันงาน) ---
    for (int i = 0; i < 1000; i++) {
        volatile int dummy = i;
        (void)dummy;
    }
...
/* ===============================================================
   Variable Time Slice Experiment (Fixed 5 seconds per time slice)
   - รันทดสอบแต่ละค่า time slice เป็นเวลา 5 วินาทีคงที่
   - แสดง Efficiency = context_switch_time / duration
   =============================================================== */
...
void app_main(void)
{
    ...
    // รันการทดลองเปรียบเทียบ time slice ก่อน
    variable_time_slice_experiment();
#endif

    // เข้าสู่โหมด time-sharing ปกติ (วนรันตลอด)
    uint64_t start_time = esp_timer_get_time();
    ...
    while (1) {
        ...
        // รายงานสถิติทุก ๆ 20 context switches (≈ 1 วินาทีเมื่อ TIME_SLICE_MS=50)
