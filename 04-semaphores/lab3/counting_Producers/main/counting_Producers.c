...
// Configuration
#define MAX_RESOURCES 5   // ใช้จากการทดลองก่อนหน้า
#define NUM_PRODUCERS 8   // ✅ เพิ่ม Producers เป็น 8
#define NUM_CONSUMERS 3   // (ยังไม่ใช้ในโค้ดนี้)
...
void app_main(void) {
    ESP_LOGI(TAG, "Counting Semaphores Lab - Experiment 3 (8 Producers)");
    ...
    if (xCountingSemaphore != NULL) {
        ...
        ESP_LOGI(TAG, "System running with:");
        ESP_LOGI(TAG, "  Resources: %d", MAX_RESOURCES);
        ESP_LOGI(TAG, "  Producers: %d", NUM_PRODUCERS);
        ESP_LOGI(TAG, "\nObserve how system handles more producer requests concurrently!");
    } else {
