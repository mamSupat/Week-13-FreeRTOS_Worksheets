...
void optimized_heavy_task(void *pvParameters) {
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "Stack remaining: %d bytes",
...
// --------------------------------
// Tasks for deletion demo
// --------------------------------
void self_deleting_task(void *pvParameters) {
    int *life = (int *)pvParameters;
    for (int i = *life; i > 0; i--) {
        ESP_LOGI(TAG, "Countdown: %d", i);
        vTaskDelay(pdMS_TO_TICKS(1000));
    }
    ESP_LOGI(TAG, "Self-deleting task going DELETED");
    vTaskDelete(NULL);
}
...
void external_delete_task(void *pvParameters) {
    int count = 0;
    while (1) {
        ESP_LOGI(TAG, "External delete task run: %d", count++);
        vTaskDelay(pdMS_TO_TICKS(1000));
    }
}
...
// --------------------------------
void app_main(void) {
    ESP_LOGI(TAG, "=== FreeRTOS Task States + Stack Detection Demo ===");
