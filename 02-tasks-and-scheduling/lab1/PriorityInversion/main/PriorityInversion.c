...
volatile bool shared_resource_busy = false;   // ใช้ใน Step 3
...
void priority_inversion_low(void *pvParameters)
{
    while (1)
    {
        if (priority_test_running)
        {
            ...
            vTaskDelay(pdMS_TO_TICKS(2000));   // จำลองใช้ resource นาน
    ...
    while (1)
    {
        if (gpio_get_level(BUTTON_PIN) == 0)
        {
            if (!priority_test_running)
            {
                ...
                ESP_LOGW(TAG, "=== PRIORITY INVERSION DEMO ===");
                priority_test_running = true;
                vTaskDelay(pdMS_TO_TICKS(8000));
                priority_test_running = false;
                ESP_LOGW(TAG, "=== PRIORITY INVERSION DONE ===");
            }
        }
...
void app_main(void)
{
    ESP_LOGI(TAG, "=== FreeRTOS Priority, Round-Robin & Inversion Demo ===");
