...
static const char *TAG = "MORE_PRODUCERS";

// LED pins
#define LED_PRODUCER_1 GPIO_NUM_2
#define LED_PRODUCER_2 GPIO_NUM_4
#define LED_PRODUCER_3 GPIO_NUM_5
#define LED_PRODUCER_4 GPIO_NUM_15   // ⭐ เพิ่มสำหรับ Producer คนที่ 4
...
// ------------------ Main ------------------
void app_main(void) {
    ESP_LOGI(TAG, "More Producers System Starting...");
    ...
    gpio_set_direction(LED_PRODUCER_4, GPIO_MODE_OUTPUT); // ⭐ เพิ่ม
    ...
    if (xProductQueue && xPrintMutex) {
        ...
        xTaskCreate(producer_task, "Producer4", 3072, &p4, 3, NULL); // ⭐ เพิ่ม

        // 👷 Consumers (เหมือนเดิม)
        ...
        ESP_LOGI(TAG, "System running with 4 producers, 2 consumers.");
    } else {
